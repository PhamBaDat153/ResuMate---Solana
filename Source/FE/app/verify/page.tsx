    ...
    <main className="min-h-screen bg-background">
      <div className="mx-auto max-w-4xl">
        ...
        <section className="rounded-2xl border border-border-low bg-card p-6">
          ...
          <label className="mt-4 block text-sm font-medium">Mã truy cập tài liệu<textarea value={wrappedKey} onChange={(event) => setWrappedKey(event.target.value)} className="mt-2 min-h-24 w-full rounded-lg border border-border-low bg-card px-3 py-2 font-mono text-xs" placeholder="Mã truy cập tài liệu đã mã hóa" /></label>
          ...
          <button type="button" onClick={() => void verify()} disabled={state === 'verifying' || !credentialAddress || !wrappedKey || !passphrase} className="mt-5 rounded-lg bg-foreground px-4 py-2 font-medium text-background disabled:opacity-50">{state === 'verifying' ? 'Đang xác minh...' : 'Xác minh chứng nhận'}</button>
