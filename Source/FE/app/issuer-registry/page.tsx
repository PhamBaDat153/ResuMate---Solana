  ...
  return (
    <main className="min-h-screen bg-background">
      <div className="mx-auto max-w-5xl">
        <header className="mb-8"><p className="text-sm font-medium uppercase tracking-[0.2em] text-muted">Quản trị hệ thống</p><h1 className="mt-3 text-4xl font-semibold tracking-tight">Quản trị đơn vị cấp</h1><p className="mt-3 max-w-2xl leading-7 text-muted">Quản lý danh sách các đơn vị được phép cấp chứng nhận và kiểm tra trạng thái của họ.</p></header>
        <section className="rounded-2xl border border-border-low bg-card p-6 shadow-[0_20px_80px_-50px_rgba(0,0,0,0.35)]">
          {!connectedWallet ? <div className="flex flex-col gap-4"><p className="text-muted">Kết nối ví Solana để đọc hoặc quản trị issuer registry.</p>{wallets.length > 0 ? wallets.map((wallet) => <button key={wallet.name} type="button" onClick={() => connect.dispatch(wallet)} disabled={connect.isRunning} className="w-fit rounded-lg bg-foreground px-4 py-2 font-medium text-background disabled:opacity-50">{connect.isRunning ? 'Đang kết nối...' : `Kết nối ${wallet.name}`}</button>) : <p className="text-sm text-muted">Không tìm thấy ví tương thích Wallet Standard.</p>}</div> : <div className="flex flex-col gap-6">
             <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm text-muted">Ví đang kết nối</p><p className="mt-1 break-all font-mono text-xs">{connectedWallet.account.address}</p></div><button type="button" onClick={() => disconnect.dispatch()} className="rounded-lg border border-border-low px-3 py-2 text-sm">Ngắt kết nối</button></div>
            ...
            {registry && <div className="rounded-xl border border-border-low p-4"><p className="text-sm text-muted">Registry authority</p><p className="mt-1 break-all font-mono text-xs">{registry.authority}</p><p className="mt-3 font-medium">{isAuthority ? 'Bạn là Registry Authority.' : 'Wallet này chỉ có quyền xem, không phải Registry Authority.'}</p></div>}
              ...
              <section className="border-t border-border-low pt-6" aria-labelledby="issuer-list-title">
                 <div className="flex flex-wrap items-center justify-between gap-3"><h2 id="issuer-list-title" className="text-xl font-semibold">Tất cả đơn vị cấp</h2><button type="button" onClick={() => void loadRegistry()} className="rounded-lg border border-border-low px-3 py-2 text-sm">Tải lại</button></div>
                ...
                {issuerListState === 'error' && <div className="mt-3 rounded-xl border border-red-500/30 bg-red-500/10 p-4" role="alert">{issuerListError}</div>}
                 {issuerListState === 'ready' && (issuers.length === 0 ? <p className="mt-3 text-sm text-muted">Chưa có đơn vị cấp hợp lệ.</p> : <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead className="border-b border-border-low text-muted"><tr><th className="px-3 py-2">Đơn vị cấp</th><th className="px-3 py-2">Tài khoản</th><th className="px-3 py-2">Loại đơn vị</th><th className="px-3 py-2">Trạng thái</th></tr></thead><tbody>{issuers.map((item) => <tr key={item.address} className="border-b border-border-low/60"><td className="break-all px-3 py-3 font-mono text-xs">{item.issuer}</td><td className="break-all px-3 py-3 font-mono text-xs">{item.address}</td><td className="px-3 py-3"><span>{ISSUER_TYPE_LABELS[item.issuerType] ?? `Loại chưa biết (${item.issuerType})`}</span><span className="mt-1 block text-xs text-muted">Mã: {item.issuerType}</span></td><td className="px-3 py-3">{item.isActive ? 'Đang hoạt động' : 'Đang tạm dừng'}</td></tr>)}</tbody></table></div>)}
              </section>
              {registry && isAuthority && <div className="grid gap-6 md:grid-cols-2">
               <section className="border-t border-border-low pt-6"><h2 className="text-xl font-semibold">Thêm đơn vị cấp</h2><label className="mt-4 block text-sm font-medium">Địa chỉ ví đơn vị cấp<input value={issuerKey} onChange={(event) => setIssuerKey(event.target.value)} className="mt-2 w-full rounded-lg border border-border-low bg-card px-3 py-2 font-mono text-xs" placeholder="Địa chỉ ví của đơn vị cấp" /></label><label className="mt-4 block text-sm font-medium">Loại đơn vị<select value={issuerType} onChange={(event) => setIssuerType(event.target.value)} className="mt-2 w-full rounded-lg border border-border-low bg-card px-3 py-2"><option value="1">Trường đại học</option><option value="2">Doanh nghiệp</option><option value="3">Tổ chức chứng nhận</option><option value="4">Cơ quan nhà nước</option><option value="5">Hiệp hội nghề nghiệp</option></select></label><button type="button" onClick={handleRegister} disabled={!connectedWallet.signer || !issuerKey.trim() || action !== null} className="mt-4 rounded-lg bg-foreground px-4 py-2 font-medium text-background disabled:opacity-50">{action === 'register' ? 'Đang thêm...' : 'Thêm đơn vị cấp'}</button></section>
               <section className="border-t border-border-low pt-6"><h2 className="text-xl font-semibold">Kiểm tra đơn vị cấp</h2><button type="button" onClick={() => void loadSelectedIssuer()} disabled={!issuerKey.trim() || action !== null} className="mt-4 rounded-lg border border-border-low px-4 py-2 text-sm">Tra cứu</button>{issuerState === 'loading' && <p className="mt-3 text-sm text-muted">Đang tra cứu...</p>}{issuerError && <p className="mt-3 text-sm text-red-600" role="alert">{issuerError}</p>}{issuer && issuerState === 'ready' && <div className="mt-4 rounded-xl bg-cream/40 p-4"><p className="break-all font-mono text-xs">{issuer.issuer}</p><p className="mt-2">Loại đơn vị: {issuer.issuerType} · {issuer.isActive ? 'Đang hoạt động' : 'Đang tạm dừng'}</p><button type="button" onClick={handleToggle} disabled={action !== null || !connectedWallet.signer} className="mt-4 rounded-lg bg-foreground px-4 py-2 font-medium text-background disabled:opacity-50">{action === 'toggle' ? 'Đang cập nhật...' : issuer.isActive ? 'Tạm dừng đơn vị' : 'Bật lại đơn vị'}</button></div>}</section>
            </div>}
          </div>}
        </section>
      </div>
